-- ============================================================================
-- FIFA Score Tracker — edit a logged match
--
-- Run after 0001_init.sql, 0002_seed_teams.sql, 0003_groups.sql,
-- 0004_team_logos.sql, 0005_leagues.sql.
--
-- Ordinary matches were already editable by their logger through the plain
-- `matches` UPDATE policy (0001_init.sql / re-declared in 0005_leagues.sql).
-- What was missing was a UI, and a safe path for the one case the plain
-- policy deliberately blocks: a match linked to a league_fixtures row (see
-- 0005_leagues.sql's "Match-edit lock" comment) — those can only be
-- rewritten through this RPC, never through a raw PostgREST PATCH, so
-- correcting a league score can never silently drift the fixture/standings/
-- bracket bookkeeping the way an unmediated edit could.
--
-- Design notes:
--   * Only the original logger (created_by) may edit a match — same
--     permission this app has always used, league or not.
--   * A non-league match, or a league round-robin-stage match whose league
--     hasn't generated a knockout bracket yet, is always safe to correct:
--     standings are computed live from player_match_results, so nothing
--     needs to be recomputed by hand.
--   * A plain round-robin league's champion is a simple derived value (no
--     bracket), so if the league had already finished, its champion is
--     recomputed from the corrected standings — cheap and always safe.
--   * A round_robin_knockout league's bracket is seeded once, from the
--     final round-robin standings, the moment the group stage completes
--     (_generate_league_knockout). Correcting a round-robin score after
--     that point could mean the wrong players were ever seeded — there is
--     no well-defined automatic fix once real knockout fixtures already
--     exist, so that specific edit is rejected with a clear error instead
--     of silently leaving a bracket that doesn't match its own standings.
--   * A knockout fixture's score can be corrected even after the fact,
--     including flipping who won: if the winner changes and they haven't
--     been advanced into the next round yet, we just move the correction
--     forward and re-wire the next round's slot. But once someone has
--     actually played in a fixture the old winner advanced them into, the
--     edit is rejected instead of silently unwinding that real, separately-
--     logged match — cascading, automatic undo of someone else's already-
--     played result is exactly the kind of surprise this app avoids.
-- ============================================================================
...
begin
  ...
  if m.created_by <> auth.uid() then
    raise exception 'Only the person who logged this match can edit it.';
  end if;
...
grant execute on function public.edit_match(uuid, smallint, smallint, int, int, timestamptz, text, uuid) to authenticated;
