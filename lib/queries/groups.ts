import type { Client } from "./matches";
import type {
  Group,
  GroupLeaderboardRow,
  GroupMemberSummary,
  GroupRole,
} from "@/types/database.types";

export type MyGroup = {
  role: GroupRole;
  joined_at: string;
  group: Group;
};

/** Every group the current user belongs to, most recently joined first. */
export async function getMyGroups(supabase: Client): Promise<MyGroup[]> {
  const { data, error } = await supabase
    .from("group_members")
    .select("role, joined_at, group:groups(*)")
    .order("joined_at", { ascending: false })
    .returns<MyGroup[]>();

  if (error) throw error;
  return data ?? [];
}

export async function getGroup(
  supabase: Client,
  groupId: string,
): Promise<Group | null> {
  const { data, error } = await supabase
    .from("groups")
    .select("*")
    .eq("id", groupId)
    .maybeSingle();

  if (error) throw error;
  return data;
}

/** Roster for one group, each member's record against the caller. */
export async function getGroupMembers(
  supabase: Client,
  groupId: string,
): Promise<GroupMemberSummary[]> {
  const { data, error } = await supabase.rpc("get_group_members", {
    p_group_id: groupId,
  });
  if (error) throw error;
  return data ?? [];
}

/**
 * Standings for one group. The RPC computes points as 3W + D; ordering
 * happens here because PostgREST can only sort by columns, and the tiebreak
 * chain is points, then goal difference, then goals scored.
 */
export async function getGroupLeaderboard(
  supabase: Client,
  groupId: string,
): Promise<GroupLeaderboardRow[]> {
  const { data, error } = await supabase.rpc("get_group_leaderboard", {
    p_group_id: groupId,
  });
  if (error) throw error;

  const rows = data ?? [];
  return [...rows].sort((a, b) => {
    if (b.points !== a.points) return b.points - a.points;
    if (b.goal_difference !== a.goal_difference) {
      return b.goal_difference - a.goal_difference;
    }
    if (b.goals_for !== a.goals_for) return b.goals_for - a.goals_for;
    return a.username.localeCompare(b.username);
  });
}

/**
 * Creates a group and seats the caller as its owner, atomically.
 *
 * create_group() is declared `returns public.groups` (a single row, not
 * SETOF), so PostgREST hands back one object rather than an array here.
 */
export async function createGroup(
  supabase: Client,
  name: string,
): Promise<Group> {
  const { data, error } = await supabase.rpc("create_group", {
    p_name: name,
  });
  if (error) throw error;
  return data;
}

/**
 * Joins a group by invite code. Re-joining (a double-tap, or a stale link
 * clicked twice) is a harmless no-op on the database side, not an error.
 */
export async function joinGroup(
  supabase: Client,
  inviteCode: string,
): Promise<Group> {
  const { data, error } = await supabase.rpc("join_group", {
    p_invite_code: inviteCode,
  });
  if (error) throw error;
  return data;
}

/** Leaves a group. Past matches are kept. The owner cannot leave (RLS). */
export async function leaveGroup(
  supabase: Client,
  groupId: string,
  userId: string,
) {
  const { error } = await supabase
    .from("group_members")
    .delete()
    .eq("group_id", groupId)
    .eq("user_id", userId);
  if (error) throw error;
}

/** Owner-only: removes another member. Past matches are kept. */
export async function removeMember(
  supabase: Client,
  groupId: string,
  memberId: string,
) {
  const { error } = await supabase
    .from("group_members")
    .delete()
    .eq("group_id", groupId)
    .eq("user_id", memberId);
  if (error) throw error;
}

/** Owner-only: renames the group. */
export async function renameGroup(
  supabase: Client,
  groupId: string,
  name: string,
) {
  const { error } = await supabase
    .from("groups")
    .update({ name })
    .eq("id", groupId);
  if (error) throw error;
}

/** Owner-only: invalidates the current invite code/link and issues a new one. */
export async function regenerateInviteCode(
  supabase: Client,
  groupId: string,
): Promise<string> {
  const { data, error } = await supabase.rpc("regenerate_invite_code", {
    p_group_id: groupId,
  });
  if (error) throw error;
  return data;
}
