...
/**
 * Every team, fetched once and filtered client-side.
 *
 * The table is ~160 rows and a few KB, so pulling the lot beats a debounced
 * round trip per keystroke on a phone.
 */
export async function getTeams(supabase: Client): Promise<Team[]> {
...
/** Groups teams by league, in LEAGUE_ORDER, for the grouped combobox. */
